...
type HomeStatus = "unknown" | "added" | "missed" | "unsupported" | string;
...
const LS_KEY = "homescreen_prompted_v1";
...
export function InstallCard({ t }: { t: (k: string) => string }) {
  ...
  const runAddToHome = () => {
    if (!addToHomeScreen.isAvailable()) {
      ...
      alert(t("tgNotSupported") ?? "Qo‘llab-quvvatlanmaydi");
      return;
    }
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        const status = (await checkHomeScreenStatus()) as HomeStatus;
        if (!mounted) return;
        ...
        if (status === "unknown") {
          const alreadyPrompted = localStorage.getItem(LS_KEY);
          if (!alreadyPrompted) {
            if (ios) setIosModalOpen(true);
            else runAddToHome();
